...
export async function uploadTickerCollection(
  files: FileList | File[],
  ...
  try {
    return await response.json() as TickerCollection;
  } catch {
    throw new Error("Failed to parse ticker files: invalid response");
  }
}
