...
use crate::services::daily_notes::DailyNotesError;
...
async fn note(State(state): State<AppState>, Path(date): Path<NaiveDate>) -> ApiResult<DailyNote> {
    state
        .daily_notes
        .get(date)
        .await
...
) -> Result<(StatusCode, Json<DailyNote>), (StatusCode, Json<Value>)> {
    state
        .daily_notes
        .create(input.date)
        .await
